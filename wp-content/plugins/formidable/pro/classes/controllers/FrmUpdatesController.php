...
<?php
    ...
    function queue_update($transient) {
		...
		if ( $this->is_current_version( $transient ) ) {
			...
			$force = $this->set_force_check( $version_info, $transient );
...
<a href="<?php echo esc_url( admin_url( 'admin.php?page=helpdesk-options&action=queue&_wpnonce=' . wp_create_nonce( $this->manually_queue_update() ) ) ); ?>" class="button"><?php _e( 'Check for Update', 'formidable' ) ?></a>
<?php
    }
