    ...
    <tr>
        ...
        <td>
            ...
            <p><label for="one"><input type="radio" value="one" id="one" <?php checked($post->frm_show_count, 'one') ?> name="show_count" onchange="javascript:frm_show_count(this.value)" /> <?php _e('Single Entry &mdash; display one entry', 'formidable'); ?>.</label>
                <span id="entry_select_container" <?php echo ($post->frm_show_count == 'one') ? '' : ' class="frm_hidden"'; ?>><br/>
            ...
            <div id="date_select_container" <?php echo ($post->frm_show_count == 'calendar') ? '' : ' class="frm_hidden"'; ?>>
                ...
                <select id="date_field_id" name="options[date_field_id]">
                    ...
                    <?php if (is_numeric($post->frm_form_id) && !empty($post->frm_form_id)) FrmProFieldsHelper::get_field_options($post->frm_form_id, $post->frm_date_field_id, '', "'date'"); ?>
                </select>
                <br/>
                ...
                <select id="date_field_id" name="options[edate_field_id]">
                    ...
                    <?php if (is_numeric($post->frm_form_id) && !empty($post->frm_form_id)) FrmProFieldsHelper::get_field_options($post->frm_form_id, $post->frm_edate_field_id, '', "'date','number'"); ?>
                </select>
            </div>
        </td>
    </tr>
